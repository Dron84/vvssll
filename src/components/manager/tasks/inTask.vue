        ...
        <div class="table"  style="max-height: 100vh;">
            ...
            <div class="table_body" style="max-height: 100%;" v-else>
                ...
                <div class="tableRow" v-else v-for="item in taskList">
                    <div class="centrize"><changeStatus :dropDownList="eventList" :downStatusWidth="200" @selectedItem="whatToDo($event, item.id)" :status-now="item.status"></changeStatus></div>
...
<script>
    ...
    export default {
        ...
        props:{
            HeaderCaption: {
                type: String,
                default: 'The Task assigned to me'
            }
        },
        components:{
            changeStatus,
        ...
        methods: {
            ...
            loadTaskList(){
                this.preloader=true
                axios.post(this.$store.getters.getPostUrl,'task=loadMy&my_id='+this.$store.getters.get_id)
                    .then(res=>{
