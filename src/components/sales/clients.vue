<template>
    <section>
        <div class="container">
            <sm></sm>
            <h header="All Clients" @searchInfo="searchInfo($event)" placeholder="Search Name or Email"></h>
        </div>
        <div class="container">
            <div class="companys"
                 style="grid-template-rows: 1fr;grid-template-areas: 'companys_body';grid-template-columns: 1fr;width: 100%;">
                ...
                <div v-else>
                    <div class="companys_body" v-if="users!='Sql is empty'">
                        <div class="companys_company box_shadow_hover_roundshadow" v-for="item in users">
                            <router-link :to="'/sales/clients/'+item.id" style="text-decoration: none;">
                    ...
                    <div v-else>No Clients in system</div>
                </div>
            </div>
        </div>
    </section>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            loadAllClients() {
                this.preloader = true
                this.users = this.$store.getters.get_salesManagerClients
                this.companys = this.$store.getters.get_salesManagerClientsCompanys
